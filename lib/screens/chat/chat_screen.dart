...
class _ChatScreenState extends State<ChatScreen> {
  void listenChatChannel(ChatEntity chat) {
    LaravelEcho.instance.private("chat.${chat.id}").listen('.message.sent', (e) {
      ...
      if (e is PusherEvent) {
        if (e.data != null) {
          ...
          eLog('channel: ${e.channelName}, event: ${e.eventName}');
    ...
    // LaravelEcho.instance.channel('chat.${chat.id}').listen('.message.sent',
    //     (e) {
    //   if (e is PusherEvent) {
    //     if (e.data != null) {
    //       vLog(jsonDecode(e.data!));
    ...
    //   eLog(err);
    // });
    // LaravelEcho.instance.listen('chat.${chat.id}', '.message.sent', (e) {
    //   if (e is PusherEvent) {
    //     if (e.data != null) {
    //       vLog(jsonDecode(e.data!));
    //       _handleNewMessage(jsonDecode(e.data!));
    ...
    //   }
    // }).error((err) {
    //   eLog((err as PusherEvent).channelName);
    // });
  }
        ...
        appBar: AppBar(
          title: BlocConsumer<ChatBloc, ChatState>(
            ...
            listenWhen: (previous, current) {
              return previous.selectedChat != current.selectedChat;
            },
